...
from doneproof.compiler import CONTRACT_SCHEMA, AstraCompiler
    ...
    c = CompletionContract.model_validate(
        {
            "task": "Create issue",
            "postconditions": [
                pc("github", {"repo": "acme/api", "kind": "issue", "number": None, "title": "Auth bypass"})
            ],
        }
    )
...
def test_compiler_rejects_weak_github_discovery():
    c = CompletionContract.model_validate(
        {
            "task": "Create issue",
            "postconditions": [pc("github", {"repo": "acme/api", "kind": "issue", "number": None})],
        }
    )
...
def test_compiler_accepts_gmail_and_webhook_selectors():
    gmail = CompletionContract.model_validate(
        {"task": "Send mail", "postconditions": [pc("gmail", {"subject": "Invoice", "to": "a@example.com"})]}
    )
    hook = CompletionContract.model_validate(
        {
            "task": "Refund",
            "postconditions": [pc("webhook", {"source": "erp", "event_type": "refund.completed", "object_id": "r1"})],
        }
    )
