...
def test_contract_ids_are_tenant_scoped(settings):
    ...
    a=CompletionContract.model_validate({"id":"cc_fixed","task":"Task A","postconditions":[{"id":"p1","description":"ok","provider":"unresolved","selector":{"state":{"ok":True}},"predicate":{"op":"eq","path":"ok","expected":True},"required":True}]})
    b=a.model_copy(deep=True); b.task="Task B"
...
def test_stats_include_pilot_metrics(settings):
    ...
    import asyncio
    from doneproof.store import Store
    c=CompletionContract.model_validate({'task':'metric','postconditions':[{'id':'p1','description':'ok','provider':'unresolved','selector':{'state':{'ok':True}},'predicate':{'op':'eq','path':'ok','expected':True},'required':True}]})
    store=Store(settings.db_path)
...
def test_legacy_global_contract_primary_key_is_migrated(tmp_path):
    ...
    con.execute('CREATE TABLE contracts (id TEXT PRIMARY KEY, task TEXT NOT NULL, body_json TEXT NOT NULL, created_at TEXT NOT NULL, tenant_id TEXT NOT NULL DEFAULT "default")')
