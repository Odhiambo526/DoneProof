...
def gmail_message(mid="m1", labels=None, subject="Invoice", to="alice@example.com", attachments=None):
    ...
    return {
        ...
        "internalDate": str(int(datetime(2026, 9, 4, 3, 1, tzinfo=timezone.utc).timestamp() * 1000)),
        "payload": {
            "headers": [
            ...
            "postconditions": [
                {
                    ...
                    "provider": "gmail",
                    "selector": {"message_id": None, "subject": "Invoice", "to": "alice@example.com"},
                    "predicate": {"op": "eq", "path": "location", "expected": location},
                    "required": True,
                },
                {
                    "id": "p2",
                    ...
                    "provider": "gmail",
                    "selector": {"message_id": None, "subject": "Invoice", "to": "alice@example.com"},
                    "predicate": {"op": "contains", "path": "attachment_names", "expected": "invoice.pdf"},
                    "required": True,
                },
            ],
        }
...
def run(handler, settings, c=None):
    ...
    return asyncio.run(
        VerificationEngine({"gmail": adapter}, ReceiptSigner(s), timeout_seconds=2).verify(c or contract())
    )
...
def test_gmail_duplicate_candidates_are_unknown(settings):
    one = gmail_message(mid="m1", attachments=["invoice.pdf"])
    two = gmail_message(mid="m2", attachments=["invoice.pdf"])
...
def test_unconnected_gmail_is_unknown(settings):
    ...
    r = asyncio.run(
        VerificationEngine({"gmail": adapter}, ReceiptSigner(settings), timeout_seconds=1).verify(contract())
    )
