...
def test_tenant_receipt_isolation(auth_settings):
    app=make_app(auth_settings); client=TestClient(app)
...
def test_request_body_limit_is_enforced(settings):
    ...
    r=client.post('/v1/verify',content=b'{' + b'"x":"' + b'a'*2000 + b'"}',headers={'content-type':'application/json'})
...
def test_production_requires_stable_signing_key_after_auth(settings):
    import pytest
    prod=replace(settings,env='production',api_keys={'prod-key':'acme'},signing_seed_b64=None,legacy_receipt_key='dev-only-change-me')
