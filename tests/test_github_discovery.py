...
def issue(number, title, created_at="2026-09-04T03:01:00Z", author="bot", assignees=None):
    return {"number":number,"title":title,"body":"body","state":"open","locked":False,"user":{"login":author},"assignees":[{"login":x} for x in (assignees or [])],"labels":[],"created_at":created_at,"updated_at":created_at,"closed_at":None,"html_url":f"https://github.com/acme/api/issues/{number}"}


def contract(selector, predicate=None):
    return CompletionContract.model_validate({"task":"Create issue Auth bypass","task_started_at":START.isoformat(),"postconditions":[{"id":"p1","description":"requested issue exists","provider":"github","selector":selector,"predicate":predicate or {"op":"eq","path":"title","expected":"Auth bypass"},"required":True}]})
...
def test_discovers_unique_issue_without_executor_number(settings):
    ...
    def handler(request):
        ...
        if request.url.path.endswith('/issues/77'): return httpx.Response(200,json=candidate)
...
def test_duplicate_matches_are_unknown(settings):
    ...
    def handler(request): return httpx.Response(200,json=[two,one])
    r=run(contract({"repo":"acme/api","kind":"issue","number":None,"title":"Auth bypass"}),handler,settings)
...
def test_preexisting_match_is_not_current_evidence(settings):
    ...
    def handler(request): return httpx.Response(200,json=[old])
    r=run(contract({"repo":"acme/api","kind":"issue","number":None,"title":"Auth bypass"}),handler,settings)
    assert r.verdict == Verdict.FAILED


def test_privacy_preserving_404_is_unknown(settings):
    def handler(request): return httpx.Response(404,json={"message":"Not Found"})
...
def test_transient_github_failure_retries(settings):
    ...
    def handler(request):
        calls["n"] += 1
        if calls["n"] == 1: return httpx.Response(502,json={})
        if request.url.path.endswith('/issues'): return httpx.Response(200,json=[candidate])
        return httpx.Response(200,json=candidate)
    r=run(contract({"repo":"acme/api","kind":"issue","number":None,"title":"Auth bypass"}),handler,settings)
    assert r.verdict == Verdict.VERIFIED
    assert calls["n"] >= 3
...
def test_caller_backdated_created_after_cannot_override_contract_boundary(settings):
    ...
    def handler(request):
        ...
        raise AssertionError(request.url)
    c=contract({"repo":"acme/api","kind":"issue","number":None,"title":"Auth bypass","created_after":"1999-01-01T00:00:00Z"})
