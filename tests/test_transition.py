...
def transition_contract():
    return CompletionContract.model_validate({
        ...
        "postconditions":[{
            "id":"p1","description":"Alice becomes assigned","provider":"mock","selector":{"record":"42"},
...
def test_registered_transition_from_false_to_true_is_verified(settings):
    adapter=SequenceAdapter([False,True]); engine=VerificationEngine({"mock":adapter},ReceiptSigner(settings))
...
def test_preexisting_true_state_is_not_credited_to_agent(settings):
    adapter=SequenceAdapter([True,True]); engine=VerificationEngine({"mock":adapter},ReceiptSigner(settings))
    c=transition_contract(); baseline=asyncio.run(engine.snapshot(c))
    r=asyncio.run(engine.verify(c,assurance_level="registered",baselines={x.id:x for x in baseline}))
...
def test_transition_required_without_registered_baseline_is_unknown(settings):
    adapter=SequenceAdapter([True]); engine=VerificationEngine({"mock":adapter},ReceiptSigner(settings))
